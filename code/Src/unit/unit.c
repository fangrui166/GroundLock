#include "unit.h"
#include "string.h"
#include <stdio.h>
#define bd_printf(fmt,...)  printf(fmt, ##__VA_ARGS__)

/* Table of CRC values for Corder byte */
static uint8_t CRC8_TAB[256] =
{

} ;
uint8_t CRC8(uint8_t *puchMsg, uint16_t usDataLen)
{
  uint8_t   uchCRC8 = 0x00;                         /* byte of CRC8 initialized */
  uint8_t   uIndex;                                 /* will index into CRC lookup table */

  while(usDataLen--)                                /* pass through message buffer */
  {
    uIndex  = uchCRC8 ^ *puchMsg++;                 /* calculate the CRC */
    uchCRC8 = CRC8_TAB[uIndex];
  }
  return  uchCRC8;
}

void buffer_dump(uint8_t *buf, int len)
{
	int i =0;
	for(i=0; i<len; i++){
		if(((i%8) == 0) && (i >0)) bd_printf("\n");
		bd_printf("0x%02x ",buf[i]);
	}
	bd_printf("\n");
}

void buffer_dump_decimal(uint8_t *buf, int len)
{
	int i =0;
	for(i=0; i<len; i++){
		if(((i%8) == 0) && (i >0)) bd_printf("\n");
		bd_printf("%d ",buf[i]);
	}
	bd_printf("\n");
}

uint8_t buf_is_null(uint8_t * data, int len)
{
    int i = 0;
	uint32_t sum=0, and=0xFF;
    for(i = 0; i < len; i++){
        if((data[i] != 0) && (data[i] != 0xFF))return 0;
		sum += data[i];
		and &= data[i];
    }
	if((sum ==0) | (and == 0xFF)) return 1;
    return 0;
}
uint8_t buf_is_match(uint8_t * des, uint8_t * src, int len)
{
    int i = 0;
    for(i = 0; i < len; i++){
        if(des[i] != src[i]) return 0;
    }
    return 1;
}

uint32_t get_le32(uint8_t *buf)
{
    uint32_t out;
    out = ( (((uint32_t)buf[3]) << 24)
        | (((uint32_t)buf[2]) << 16)
        | (((uint32_t)buf[1]) <<  8)
        | (((uint32_t)buf[0]))     );
    return out;
}
uint16_t get_le16(uint8_t *buf)
{
    uint16_t out;
    out = ((((uint16_t)buf[1]) <<  8)
        | (((uint16_t)buf[0]))     );
    return out;
}


